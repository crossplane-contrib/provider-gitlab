...
// ServiceAccountAccessTokenParameters define the desired state of a GitLab instance
// service account personal access token.
//
// Instance service accounts have no dedicated token endpoints; their tokens are
// managed through the personal access tokens API. In owner mode this requires an
// instance-admin ProviderConfig. In self-managed mode the provider authenticates
// as the service account itself and uses the self endpoints.
...
// A ServiceAccountAccessToken is a managed resource that represents a GitLab instance
// service account personal access token.
// This is only available on self-managed GitLab instances with at least a Premium license.
...
// +kubebuilder:resource:scope=Namespaced,categories={crossplane,managed,gitlab},shortName=isaat
