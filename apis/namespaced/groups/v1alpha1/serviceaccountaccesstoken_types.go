...
// ServiceAccountAccessTokenParameters define the desired state of a GitLab group
...
type ServiceAccountAccessTokenParameters struct {
	// GroupID is the ID of the group that owns the service account.
	// +optional
	// +immutable
...
// A ServiceAccountAccessToken is a managed resource that represents a GitLab group
// service account personal access token.
...
// +kubebuilder:resource:scope=Namespaced,categories={crossplane,managed,gitlab},shortName=saat
