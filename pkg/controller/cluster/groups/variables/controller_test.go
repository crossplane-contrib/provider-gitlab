...
import (
	...
	shared "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/groups/variables"
)
...
var (
	...
	variableValue    = "1234"
	variableType     = sharedGroupsV1alpha1.VariableTypeEnvVar
	variableEnvScope = "*"
	f                = false
)
...
func withDefaultValues() variableModifier {
	return func(pv *v1alpha1.Variable) {
		pv.Spec.ForProvider = v1alpha1.VariableParameters{
			VariableParameters: sharedGroupsV1alpha1.VariableParameters{
				GroupID:          &groupID,
				Key:              variableKey,
				Value:            &variableValue,
				Protected:        &f,
...
func withVariableType(variableType sharedGroupsV1alpha1.VariableType) variableModifier {
	return func(r *v1alpha1.Variable) {
	...
	cases := map[string]struct {
		...
		"LateInitSuccess": {
			args: args{
				...
				cr: variable(
					...
					withVariableType(sharedGroupsV1alpha1.VariableTypeEnvVar),
					withRaw(false),
				),
			},
			want: want{
		...
		"ValueSecretRefWrongKey": {
			args: args{
				...
				variable: &fake.MockClient{
					MockGetGroupVariable: func(gid interface{}, key string, opt *gitlab.GetGroupVariableOptions, options ...gitlab.RequestOptionFunc) (*gitlab.GroupVariable, *gitlab.Response, error) {
						return &gitlab.GroupVariable{}, &gitlab.Response{Response: &http.Response{StatusCode: 400}}, errors.New(shared.ErrSecretKeyNotFound)
					},
				},
				cr: variable(
					withGroupID(groupID),
	...
	cases := map[string]struct {
		...
		"ValueSecretRefWrongKey": {
			args: args{
				...
				variable: &fake.MockClient{
					MockCreateGroupVariable: func(gid interface{}, opt *gitlab.CreateGroupVariableOptions, options ...gitlab.RequestOptionFunc) (*gitlab.GroupVariable, *gitlab.Response, error) {
						return &gitlab.GroupVariable{}, &gitlab.Response{}, errors.New(shared.ErrSecretKeyNotFound)
					},
				},
				cr: variable(
					withGroupID(groupID),
