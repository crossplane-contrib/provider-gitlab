...
import (
	...
	shared "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/groups/members"
)
...
func withStatus(s sharedGroupsV1alpha1.MemberObservation) groupModifier {
	return func(r *v1alpha1.Member) { r.Status.AtProvider = s }
}

func withSpec(s sharedGroupsV1alpha1.MemberParameters) groupModifier {
...
func TestObserve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"FailedGetRequest": {
			args: args{
				...
				cr: groupMember(
					withGroupID(),
					withSpec(sharedGroupsV1alpha1.MemberParameters{UserID: &userID, GroupID: &groupID})),
			},
			want: want{
				cr: groupMember(
					withGroupID(),
		...
		"NoUserIDandNoUserName": {
			args: args{
				...
				cr: groupMember(
					withSpec(sharedGroupsV1alpha1.MemberParameters{
						GroupID:  &groupID,
						UserID:   nil,
						UserName: nil,
					},
			...
			want: want{
				...
				result: managed.ExternalObservation{},
				err:    errors.New(shared.ErrMissingUserInfo),
			},
		},
		"NoUserIDSuccess": {
			args: args{
			...
			want: want{
				cr: groupMember(
					withGroupID(),
					withConditions(xpv1.Available()),
					withStatus(sharedGroupsV1alpha1.MemberObservation{}),
					withSpec(sharedGroupsV1alpha1.MemberParameters{
						UserName: &username,
						UserID:   &userID,
						GroupID:  &groupID,
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &shared.External{Client: tc.groupMember, UserClient: tc.user}
...
func TestCreate(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"SuccessfulCreationWithoutExpiresAt": {
			args: args{
				...
				cr: groupMember(
					withSpec(sharedGroupsV1alpha1.MemberParameters{GroupID: &ID}),
				),
			},
			want: want{
				cr: groupMember(
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &shared.External{Client: tc.groupMember}
		...
		"SuccessfulUpdate": {
			args: args{
				...
				cr: groupMember(
					withGroupID(),
					withSpec(sharedGroupsV1alpha1.MemberParameters{UserID: &userID, GroupID: &groupID}),
					withStatus(sharedGroupsV1alpha1.MemberObservation{Username: "new username"}),
				),
			},
			want: want{
				cr: groupMember(
