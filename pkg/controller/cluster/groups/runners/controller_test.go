...
func withSpec(s sharedGroupsV1alpha1.RunnerParameters) RunnerModifier {
...
func TestConnect(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"InValidInput": {
			...
			want: want{
				cr:  unexpecedItem,
				err: errors.New(shared.ErrNotRunner),
			},
		},
...
func TestObserve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"NoExternalName": {
			args: args{
				cr: runner(
					withSpec(sharedGroupsV1alpha1.RunnerParameters{GroupID: &groupID}),
				),
			},
			want: want{
				cr: runner(
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &shared.External{Client: tc.runnerClient, UserRunnerClient: tc.userRunnerClient}
	...
	cases := map[string]struct {
		...
		"MissingConnectionSecret": {
			args: args{
				userRunnerClient: &usersfake.MockClient{
					MockCreateUserRunner: func(opts *gitlab.CreateUserRunnerOptions, options ...gitlab.RequestOptionFunc) (*gitlab.UserRunner, *gitlab.Response, error) {
						return &gitlab.UserRunner{}, &gitlab.Response{}, nil
					},
				},
				cr: runner(
					withGroupID(),
			...
			want: want{
				...
				err:    errors.New(shared.ErrMissingConnectionSecret),
				result: managed.ExternalCreation{},
			},
		},
		"FailedCreation": {
