...
import (
	...
	shared "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/groups/groups"
)
...
var (
	...
	visibility         = "private"
	v1alpha1Visibility = sharedGroupsV1alpha1.VisibilityValue(visibility)

	timeNow = time.Now()

	projectCreationLevel         = "developer"
	v1alpha1ProjectCreationLevel = sharedGroupsV1alpha1.ProjectCreationLevelValue(projectCreationLevel)

	subGroupCreationLevel         = "maintainer"
	v1alpha1SubGroupCreationLevel = sharedGroupsV1alpha1.SubGroupCreationLevelValue(subGroupCreationLevel)
)
...
func withConditions(c ...xpv1.Condition) groupModifier {
	return func(cr *v1alpha1.Group) { cr.Status.Conditions = c }
}
...
func withProjectCreationLevel(s *sharedGroupsV1alpha1.ProjectCreationLevelValue) groupModifier {
	return func(r *v1alpha1.Group) { r.Spec.ForProvider.ProjectCreationLevel = s }
}

func withVisibility(s *sharedGroupsV1alpha1.VisibilityValue) groupModifier {
	return func(r *v1alpha1.Group) { r.Spec.ForProvider.Visibility = s }
}

func withSubGroupCreationLevel(s *sharedGroupsV1alpha1.SubGroupCreationLevelValue) groupModifier {
...
func withClientDefaultValues() groupModifier {
	return func(p *v1alpha1.Group) {
		...
		i := 0
		s := ""
		p.Spec.ForProvider.GroupParameters = sharedGroupsV1alpha1.GroupParameters{
			MembershipLock:                 &f,
		...
		p.Status.AtProvider = sharedGroupsV1alpha1.GroupObservation{
			ID:        &i,
			AvatarURL: &s,
			WebURL:    &s,
			FullName:  &s,
...
func withStatus(s sharedGroupsV1alpha1.GroupObservation) groupModifier {
...
func withSharedWithGroupsObservation(s []sharedGroupsV1alpha1.SharedWithGroupsObservation) groupModifier {
...
func TestConnect(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"InValidInput": {
			...
			want: want{
				cr:  unexpecedItem,
				err: errors.New(shared.ErrNotGroup),
			},
		},
...
func TestObserve(t *testing.T) {
	...
	v1alpha1VisibilityNew := sharedGroupsV1alpha1.VisibilityValue("public")
	gitlabProjectCreationLevelNew := gitlab.NoOneProjectCreation
	v1alpha1ProjectCreationLevelNew := sharedGroupsV1alpha1.ProjectCreationLevelValue("noone")
	gitlabSubGroupCreationLevelNew := gitlab.Ptr("owner")
	v1alpha1SubGroupCreationLevelNew := sharedGroupsV1alpha1.SubGroupCreationLevelValue("owner")
		...
		"LateInitSuccess": {
			...
			want: want{
				cr: group(
					...
					withStatus(sharedGroupsV1alpha1.GroupObservation{}),
					withClientDefaultValues(),
					withSharedWithGroupsObservation(
						[]sharedGroupsV1alpha1.SharedWithGroupsObservation{
							{
								GroupID:          &groupID,
								GroupName:        &name,
								GroupFullPath:    &path,
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &shared.External{Client: tc.group}
	...
	cases := map[string]struct {
		...
		"FailedCreation": {
			args: args{
				...
				cr: group(withStatus(sharedGroupsV1alpha1.GroupObservation{ID: &groupID})),
			},
			want: want{
				cr:  group(withStatus(sharedGroupsV1alpha1.GroupObservation{ID: &groupID})),
	...
	cases := map[string]struct {
		...
		"SuccessfulUpdate": {
			args: args{
				...
				cr: group(withStatus(sharedGroupsV1alpha1.GroupObservation{ID: &groupID}), withExternalName("1234")),
			},
			want: want{
				cr: group(
					withStatus(sharedGroupsV1alpha1.GroupObservation{ID: &groupID}),
		...
		"UnsharedWithGroups": {
			args: args{
				...
				cr: group(
					withSharedWithGroups([]v1alpha1.SharedWithGroups{
						{SharedWithGroups: sharedGroupsV1alpha1.SharedWithGroups{GroupID: &groupIDtwo}},
					}),
				),
			},
			want: want{
		...
		"SharedWithGroupsFailed": {
			...
			want: want{
				...
				err:    errors.Wrapf(errBoom, shared.ErrShareFailed, groupID),
				result: managed.ExternalUpdate{},
			},
		},
		"UnsharedWithGroupsFailed": {
			...
			want: want{
				cr:     group(),
				result: managed.ExternalUpdate{},
				err:    errors.Wrapf(errBoom, shared.ErrUnshareFailed, groupID),
			},
		},
		"FailedUpdate": {
			args: args{
			...
			want: want{
				cr:  group(withStatus(sharedGroupsV1alpha1.GroupObservation{ID: &groupID})),
				err: errors.Wrap(errBoom, shared.ErrUpdateFailed),
			},
		},
	}
	for name, tc := range cases {
...
func TestDelete(t *testing.T) {
		...
		"SuccessfulPermanentlyDeletion": {
			args: args{
				...
				cr: group(
					...
					withStatus(sharedGroupsV1alpha1.GroupObservation{FullPath: gitlab.Ptr("path/to/group")})),
			},
			want: want{
				cr: group(
					withExternalName("0"),
		...
		"SuccessfulPermanentlyTopLevelGroupDeletion": {
			args: args{
				...
				cr: group(
					...
					withStatus(sharedGroupsV1alpha1.GroupObservation{FullPath: gitlab.Ptr("top-level-group")})),
			},
			want: want{
				cr: group(
					withExternalName("0"),
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.calls, recordedCalls, test.EquateConditions()); diff != "" {
				t.Errorf("r: -want, +got:\n%s", diff)
			}
		})
	}
