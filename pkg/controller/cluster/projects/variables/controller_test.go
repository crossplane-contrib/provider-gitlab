...
import (
	...
	shared "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/projects/variables"
)
...
var (
	...
	variableValue    = "1234"
	variableType     = sharedProjectsV1alpha1.VariableTypeEnvVar
	variableEnvScope = "*"
	f                = false
)
...
func withConditions(c ...xpv1.Condition) variableModifier {
	return func(r *v1alpha1.Variable) { r.Status.Conditions = c }
}

func withDefaultValues() variableModifier {
	return func(pv *v1alpha1.Variable) {
		pv.Spec.ForProvider.VariableParameters = sharedProjectsV1alpha1.VariableParameters{
			ProjectID:        &projectID,
			Key:              variableKey,
			Value:            &variableValue,
			Protected:        &f,
...
func withVariableType(variableType sharedProjectsV1alpha1.VariableType) variableModifier {
	return func(r *v1alpha1.Variable) {
...
func TestObserve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"LateInitSuccess": {
			args: args{
				...
				cr: variable(
					...
					withVariableType(sharedProjectsV1alpha1.VariableTypeEnvVar),
					withRaw(false),
				),
			},
			want: want{
		...
		"ValueSecretRefWrongKey": {
			args: args{
				...
				variable: &fake.MockClient{
					MockGetVariable: func(pid interface{}, key string, opt *gitlab.GetProjectVariableOptions, options ...gitlab.RequestOptionFunc) (*gitlab.ProjectVariable, *gitlab.Response, error) {
						return &gitlab.ProjectVariable{}, &gitlab.Response{Response: &http.Response{StatusCode: 400}}, errors.New(shared.ErrSecretKeyNotFound)
					},
				},
				cr: variable(
					withProjectID(projectID),
			...
			want: want{
				...
				err: errors.Wrap(errors.New(shared.ErrSecretKeyNotFound), shared.ErrGetFailed),
			},
		},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &shared.External{Client: tc.variable, Kube: tc.kube}
	...
	cases := map[string]struct {
		...
		"ValueSecretRefWrongKey": {
			args: args{
				...
				variable: &fake.MockClient{
					MockCreateVariable: func(pid interface{}, opt *gitlab.CreateProjectVariableOptions, options ...gitlab.RequestOptionFunc) (*gitlab.ProjectVariable, *gitlab.Response, error) {
						return &gitlab.ProjectVariable{}, &gitlab.Response{}, errors.New(shared.ErrSecretKeyNotFound)
					},
				},
				cr: variable(
					withProjectID(projectID),
			...
			want: want{
				...
				err: errors.Wrap(errors.New(shared.ErrSecretKeyNotFound), shared.ErrCreateFailed),
			},
		},
	}
	for name, tc := range cases {
	...
	cases := map[string]struct {
		...
		"ValueSecretRefWrongKey": {
			...
			want: want{
				...
				err: errors.Wrap(errors.New(shared.ErrSecretKeyNotFound), shared.ErrUpdateFailed),
			},
		},
	}
