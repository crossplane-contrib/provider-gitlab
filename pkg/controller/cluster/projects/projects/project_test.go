...
import (
	...
	shared "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/projects/projects"
)
...
func withConditions(c ...xpv1.Condition) projectModifier {
	return func(r *v1alpha1.Project) { r.Status.Conditions = c }
}
...
func withStatus(s sharedProjectsV1alpha1.ProjectObservation) projectModifier {
	return func(r *v1alpha1.Project) { r.Status.AtProvider = s }
}

func withSpec(s sharedProjectsV1alpha1.ProjectParameters) projectModifier {
	return func(r *v1alpha1.Project) { r.Spec.ForProvider.ProjectParameters = s }
}

func withProjectPushRules(pr *sharedProjectsV1alpha1.PushRules) projectModifier {
...
func withClientDefaultValues() projectModifier {
	return func(p *v1alpha1.Project) {
		f := false
		i := 0
		p.Spec.ForProvider.ProjectParameters = sharedProjectsV1alpha1.ProjectParameters{
...
func TestConnect(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"InValidInput": {
			...
			want: want{
				cr:  unexpecedItem,
				err: errors.New(shared.ErrNotProject),
			},
		},
...
func TestObserve(t *testing.T) {
		...
		"PushRulesNotAvailable404": {
			...
			want: want{
				cr: project(
					...
					withStatus(sharedProjectsV1alpha1.ProjectObservation{}),
				),
				result: managed.ExternalObservation{
					ResourceExists:          true,
		...
		"SuccessfulAvailable": {
			args: args{
				...
				cr: project(
					withClientDefaultValues(),
					withExternalName(extName),
					withProjectPushRules(&sharedProjectsV1alpha1.PushRules{
	...
	f := false
	i := 0
	al := sharedProjectsV1alpha1.PublicAccessControl
	tags := []string{"tag-1 new", "tag-2 new"}
	topics := []string{"tag-1 new", "tag-2 new"}
	mergeMethod := sharedProjectsV1alpha1.FastForwardMerge
	s := "default string"
	visibility := sharedProjectsV1alpha1.PublicVisibility

	projectParameters := sharedProjectsV1alpha1.ProjectParameters{
		Name:                             &s,
		Path:                             &s,
		DefaultBranch:                    &s,
		Description:                      &s,
		...
		AllowMergeOnSkippedPipeline:      &f,
		CIForwardDeploymentEnabled:       &f,
		PushRules: &sharedProjectsV1alpha1.PushRules{
			AuthorEmailRegex:           ptr.To(""),
			BranchNameRegex:            ptr.To(""),
			CommitCommitterCheck:       ptr.To(false),
			CommitCommitterNameCheck:   ptr.To(false),
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &shared.External{Client: tc.project, Kube: tc.kube}
	...
	cases := map[string]struct {
		...
		"SuccessfulEditProject": {
			args: args{
				...
				cr: project(withStatus(sharedProjectsV1alpha1.ProjectObservation{ID: 1234})),
			},
			want: want{
				cr: project(withStatus(sharedProjectsV1alpha1.ProjectObservation{ID: 1234})),
			},
		...
		"SuccessfulPermanentlyDeletion": {
			args: args{
				...
				cr: project(
					...
					withStatus(sharedProjectsV1alpha1.ProjectObservation{PathWithNamespace: "path/to/project"}),
				),
			},
			want: want{
				cr: project(
