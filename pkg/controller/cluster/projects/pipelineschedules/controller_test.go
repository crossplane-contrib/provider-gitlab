...
import (
	...
	shared "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/projects/pipelineschedules"
)
...
var (
	...
	extName          = id
	projectID        = 123456
	standardPsParams = sharedProjectsV1alpha1.PipelineScheduleParameters{
		ProjectID:    &projectID,
		Description:  s,
		Ref:          s,
		Cron:         s,
	...
	psParams = sharedProjectsV1alpha1.PipelineScheduleParameters{
		ProjectID:   &projectID,
		Description: s,
		Ref:         s,
		Cron:        s,
	...
	pv1 = &sharedProjectsV1alpha1.PipelineVariable{
		Key:          "testKey1",
		Value:        "testValue1",
		VariableType: &s,
	}
	pv2 = &sharedProjectsV1alpha1.PipelineVariable{
		Key:          "testKey2",
		Value:        "testValue2",
		VariableType: &s,
	}
	pv2Update = &sharedProjectsV1alpha1.PipelineVariable{
		Key:          "testKey2",
...
func withExternalName(s int) psModifier {
	return func(ps *v1alpha1.PipelineSchedule) { meta.SetExternalName(ps, strconv.Itoa(s)) }
}

func withParams(p sharedProjectsV1alpha1.PipelineScheduleParameters) psModifier {
	return func(ps *v1alpha1.PipelineSchedule) { ps.Spec.ForProvider.PipelineScheduleParameters = p }
}
...
func withVariables(varr ...*sharedProjectsV1alpha1.PipelineVariable) psModifier {
	return func(ps *v1alpha1.PipelineSchedule) {
		ps.Spec.ForProvider.Variables = make([]sharedProjectsV1alpha1.PipelineVariable, len(varr))
		for i, v := range varr {
...
func TestObserve(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"NoProjectID": {
			...
			expected: expected{
				cr:     buildPs(withExternalName(extName)),
				result: managed.ExternalObservation{},
				err:    errors.New(shared.ErrNoProjectID),
			},
		},
		"GetFail400": {
			args: args{
				...
				client: &fake.MockClient{
					MockGetPipelineSchedule: func(pid interface{}, schedule int, options ...gitlab.RequestOptionFunc) (*gitlab.PipelineSchedule, *gitlab.Response, error) {
						return &gitlab.PipelineSchedule{}, nil, errors.New(shared.ErrNotPipelineSchedule)
					},
				},
			},
			expected: expected{
				...
				result: managed.ExternalObservation{},
				err:    errors.Wrap(errors.New(shared.ErrNotPipelineSchedule), shared.ErrGetPipelineSchedule),
			},
		},
		"GetFail404": {
			args: args{
				...
				client: &fake.MockClient{
					MockGetPipelineSchedule: func(pid interface{}, schedule int, options ...gitlab.RequestOptionFunc) (*gitlab.PipelineSchedule, *gitlab.Response, error) {
						...
						}, errors.New(shared.ErrNotPipelineSchedule)
					},
				},
			},
			expected: expected{
		...
		"SuccessLateInitializedFalse": {
			args: args{
				client: &fake.MockClient{
					MockGetPipelineSchedule: func(pid interface{}, schedule int, options ...gitlab.RequestOptionFunc) (*gitlab.PipelineSchedule, *gitlab.Response, error) {
						return &gitlab.PipelineSchedule{Variables: gPvArr}, &gitlab.Response{}, nil
					},
				},
				cr: buildPs(
					withExternalName(extName),
		...
		"SuccessUpToDateFalse": {
			args: args{
				client: &fake.MockClient{
					MockGetPipelineSchedule: func(pid interface{}, schedule int, options ...gitlab.RequestOptionFunc) (*gitlab.PipelineSchedule, *gitlab.Response, error) {
						return &gitlab.PipelineSchedule{Cron: "cron"}, &gitlab.Response{}, nil
					},
				},
				cr: buildPs(
					withExternalName(extName),
	...
	for tn, tc := range tcs {
		t.Run(tn, func(t *testing.T) {
			victim := &shared.External{Client: tc.client, Kube: tc.kube}
...
func TestUpdate(t *testing.T) {
	...
	tcs := map[string]struct {
		...
		"NoExternalName": {
			...
			expected: expected{
				cr:     buildPs(),
				err:    errors.New(shared.ErrExternalNameMissing),
				result: managed.ExternalUpdate{},
			},
		},
		"NoProjectID": {
