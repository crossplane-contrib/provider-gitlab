...
func (c *connector) Connect(ctx context.Context, mgd resource.Managed) (managed.ExternalClient, error) {
	...
	if !ok {
		return nil, errors.New(shared.ErrNotDeployKey)
	}

	config, err := clients.ResolveProviderConfig(ctx, c.kube, cr)
	if err != nil {
		return nil, err
	}

	return &shared.External{Client: c.newGitlabClientFn(*config), Kube: c.kube}, nil
}
