...
import (
	...
	sharedRunners "github.com/crossplane-contrib/provider-gitlab/pkg/controller/shared/projects/runners"
)
...
func withConditions(c ...xpv1.Condition) RunnerModifier {
	return func(cr *v1alpha1.Runner) { cr.Status.Conditions = c }
}
...
func withSpec(s sharedProjectsV1alpha1.RunnerParameters) RunnerModifier {
	return func(r *v1alpha1.Runner) { r.Spec.ForProvider.RunnerParameters = s }
}
...
func TestObserve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"NoExternalName": {
			args: args{
				cr: runner(
					withSpec(sharedProjectsV1alpha1.RunnerParameters{ProjectID: &projectID}),
				),
			},
			want: want{
				cr: runner(
		...
		"NotIDExternalName": {
			...
			want: want{
				...
				err: errors.New(sharedRunners.ErrIDNotInt),
			},
		},
		"NoProjectID": {
			args: args{
			...
			want: want{
				...
				err:    errors.New(sharedRunners.ErrMissingProjectID),
				result: managed.ExternalObservation{},
			},
		},
		"FailedGetRequest": {
			...
			want: want{
				...
				result: managed.ExternalObservation{ResourceExists: false},
				err:    errors.Wrap(errBoom, sharedRunners.ErrGetFailed),
			},
		},
		"ErrGet404": {
			args: args{
		...
		"SuccessfulAvailable": {
			...
			want: want{
				cr: runner(
					...
					withExternalName(extName),
					withSpec(sharedProjectsV1alpha1.RunnerParameters{ProjectID: &projectID}),
					// withStatus(sharedProjectsV1alpha1.RunnerObservation{Name: name}),
				),
				result: managed.ExternalObservation{
					ResourceExists:          true,
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			e := &sharedRunners.External{Client: tc.runnerClient, UserRunnerClient: tc.userRunnerClient}
	...
	cases := map[string]struct {
		...
		"NoExternalName": {
			...
			want: want{
				...
				err: errors.New(sharedRunners.ErrMissingExternalName),
			},
		},
		"NoProjectID": {
			args: args{
		...
		"FailedDeletion": {
			...
			want: want{
				...
				err: errors.Wrap(errBoom, sharedRunners.ErrDeleteFailed),
			},
		},
		"SuccessfulDeletion": {
			args: args{
	...
	cases := map[string]struct {
		...
		"MissingConnectionSecret": {
			...
			want: want{
				...
				err:    errors.New(sharedRunners.ErrMissingConnectionSecret),
				result: managed.ExternalCreation{},
			},
		},
		"FailedCreation": {
			...
			want: want{
				...
				err: errors.Wrap(errBoom, sharedRunners.ErrCreateFailed),
			},
		},
		"SuccessfulCreation": {
			args: args{
	...
	cases := map[string]struct {
		...
		"FailedUpdate": {
			...
			want: want{
				...
				err: errors.Wrap(errBoom, sharedRunners.ErrUpdateFailed),
			},
		},
		"SuccessfulUpdate": {
			args: args{
