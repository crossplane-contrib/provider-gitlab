...
func withProjectID() UserRunnerModifier {
	return func(r *v1alpha1.UserRunner) { r.Spec.ForProvider.ProjectID = &projectID }
}
...
func TestObserve(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"NoExternalName": {
			args: args{
				cr: userRunner(
					withSpec(v1alpha1.UserRunnerParameters{ProjectID: &projectID}),
				),
			},
			want: want{
				cr: userRunner(
