...
const (
	errNotGroupProject = "managed resource is not a Gitlab groupproject custom resource"
	errGetFailed       = "cannot get Gitlab GroupProject"
	errCreateFailed    = "cannot create Gitlab groupproject"
	errUpdateFailed    = "cannot update Gitlab groupproject"
	errDeleteFailed    = "cannot delete Gitlab groupproject"
)

// SetupGroupProject adds a controller that reconciles GroupProjects.
func SetupGroupProject(mgr ctrl.Manager, l logging.Logger) error {
	name := managed.ControllerName(v1alpha1.GroupProjectKind)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		For(&v1alpha1.GroupProject{}).
		Complete(managed.NewReconciler(mgr,
			resource.ManagedKind(v1alpha1.GroupProjectGroupVersionKind),
			managed.WithExternalConnecter(&connector{kube: mgr.GetClient(), newGitlabClientFn: projects.NewGroupProjectClient}),
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	cr, ok := mg.(*v1alpha1.GroupProject)
	if !ok {
		return nil, errors.New(errNotGroupProject)
	}
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	cr, ok := mg.(*v1alpha1.GroupProject)
	if !ok {
		return managed.ExternalObservation{}, errors.New(errNotGroupProject)
	}
	...
	groupProjectID, err := strconv.Atoi(externalName)
	if err != nil {
		return managed.ExternalObservation{}, errors.New(errNotGroupProject)
	}

	prj, _, _ := e.client.GetProject(groupProjectID, nil)
	if prj == nil {
		return managed.ExternalObservation{ResourceExists: false}, errors.Wrap(resource.Ignore(projects.IsErrorGroupProjectNotFound, err), errGetFailed)
	}

	current := cr.Spec.ForProvider.DeepCopy()
	lateInitialize(&cr.Spec.ForProvider, prj)

	cr.Status.AtProvider = projects.GenerateGroupProjectObservation(prj)
	cr.Status.SetConditions(xpv1.Available())

	return managed.ExternalObservation{
		ResourceExists:          true,
		ResourceUpToDate:        isGroupProjectUpToDate(&cr.Spec.ForProvider, prj),
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	cr, ok := mg.(*v1alpha1.GroupProject)
	if !ok {
		return managed.ExternalCreation{}, errors.New(errNotGroupProject)
	}

	prj, _, err := e.client.CreateProject(
		projects.GenerateCreateGroupProjectOptions(cr.Name, &cr.Spec.ForProvider),
...
func (e *external) Update(ctx context.Context, mg resource.Managed) (managed.ExternalUpdate, error) {
	cr, ok := mg.(*v1alpha1.GroupProject)
	if !ok {
		return managed.ExternalUpdate{}, errors.New(errNotGroupProject)
	}

	_, _, err := e.client.EditProject(
		meta.GetExternalName(cr),
		projects.GenerateEditGroupProjectOptions(cr.Name, &cr.Spec.ForProvider),
		gitlab.WithContext(ctx),
	)
...
func (e *external) Delete(ctx context.Context, mg resource.Managed) error {
	cr, ok := mg.(*v1alpha1.GroupProject)
	if !ok {
		return errors.New(errNotGroupProject)
	}
...
// lateInitialize fills the empty fields in the groupproject spec with the
// values seen in gitlab.Project.
func lateInitialize(in *v1alpha1.GroupProjectParameters, project *gitlab.Project) { // nolint:gocyclo
	if project == nil {
		return
	}
...
// isGroupProjectUpToDate checks whether there is a change in any of the modifiable fields.
func isGroupProjectUpToDate(p *v1alpha1.GroupProjectParameters, g *gitlab.Project) bool { // nolint:gocyclo
