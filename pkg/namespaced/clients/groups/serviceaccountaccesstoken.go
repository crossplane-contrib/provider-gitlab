...
// ServiceAccountAccessTokenClient defines GitLab group service account personal
// access token operations.
type ServiceAccountAccessTokenClient interface {
	// Owner-authenticated endpoints (the ProviderConfig is a group owner).
	ListServiceAccountPersonalAccessTokens(gid any, serviceAccount int64, opt *gitlab.ListServiceAccountPersonalAccessTokensOptions, options ...gitlab.RequestOptionFunc) ([]*gitlab.PersonalAccessToken, *gitlab.Response, error)
	CreateServiceAccountPersonalAccessToken(gid any, serviceAccount int64, opt *gitlab.CreateServiceAccountPersonalAccessTokenOptions, options ...gitlab.RequestOptionFunc) (*gitlab.PersonalAccessToken, *gitlab.Response, error)
	RevokeServiceAccountPersonalAccessToken(gid any, serviceAccount, token int64, options ...gitlab.RequestOptionFunc) (*gitlab.Response, error)
	RotateServiceAccountPersonalAccessToken(gid any, serviceAccount, token int64, opt *gitlab.RotateServiceAccountPersonalAccessTokenOptions, options ...gitlab.RequestOptionFunc) (*gitlab.PersonalAccessToken, *gitlab.Response, error)
...
// NewServiceAccountAccessTokenClient returns a new GitLab service account access token service.
func NewServiceAccountAccessTokenClient(cfg common.Config) ServiceAccountAccessTokenClient {
	...
	return &serviceAccountAccessTokenClient{
		GroupsServiceInterface:               git.Groups,
		PersonalAccessTokensServiceInterface: git.PersonalAccessTokens,
	}
}

// serviceAccountAccessTokenClient composes group and personal access token services.
type serviceAccountAccessTokenClient struct {
	gitlab.GroupsServiceInterface
...
func GetServiceAccountAccessToken(c ServiceAccountAccessTokenClient, gid any, serviceAccount, tokenID int64, options ...gitlab.RequestOptionFunc) (*gitlab.PersonalAccessToken, *gitlab.Response, error) {
	...
	for {
		tokens, res, err := c.ListServiceAccountPersonalAccessTokens(gid, serviceAccount, opt, options...)
		if err != nil {
			return nil, res, err
		}
		lastRes = res
...
func GenerateCreateServiceAccountAccessTokenOptions(p *v1alpha1.ServiceAccountAccessTokenParameters) *gitlab.CreateServiceAccountPersonalAccessTokenOptions {
	accesstoken := &gitlab.CreateServiceAccountPersonalAccessTokenOptions{
		Name:   &p.Name,
		Scopes: &p.Scopes,
	}
...
// GenerateRotateServiceAccountAccessTokenOptions generates rotation options.
func GenerateRotateServiceAccountAccessTokenOptions(p *v1alpha1.ServiceAccountAccessTokenParameters) *gitlab.RotateServiceAccountPersonalAccessTokenOptions {
	accesstoken := &gitlab.RotateServiceAccountPersonalAccessTokenOptions{}
