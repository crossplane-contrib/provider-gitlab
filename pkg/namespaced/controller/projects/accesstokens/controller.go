...
func SetupAccessToken(mgr ctrl.Manager, o controller.Options) error {
	...
	reconcilerOpts := []managed.ReconcilerOption{
		managed.WithExternalConnector(&connector{kube: mgr.GetClient(), newGitlabClientFn: projects.NewAccessTokenClient}),
...
func (e *external) observeSelf(ctx context.Context, cr *v1alpha1.AccessToken) (managed.ExternalObservation, error) {
	...
	// authenticates with. The self endpoints are not scoped by project, and the
	// self-inform response does not carry the token's project binding, so the
	// only spec-derived identity signal is the token name. Refuse to manage a
	// token whose name does not match, to catch a miswired credentials secret.
	...
	cr.Status.AtProvider = projects.GenerateProjectAccessTokenObservationFromPAT(at)
	...
	if projects.ShouldRotateAccessTokenFromPAT(&cr.Spec.ForProvider, at) {
		return managed.ExternalObservation{ResourceExists: false}, nil
	}
...
func (e *external) observeOwner(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	at, res, err := e.client.GetProjectAccessToken(*cr.Spec.ForProvider.ProjectID, int64(accessTokenID), gitlab.WithContext(ctx))
	if err != nil {
