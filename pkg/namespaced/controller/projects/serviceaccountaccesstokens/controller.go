...
const (
	errNotServiceAccountAccessToken = "managed resource is not a Gitlab project service account access token custom resource"
...
// project ServiceAccountAccessTokens.
func SetupServiceAccountAccessToken(mgr ctrl.Manager, o controller.Options) error {
	...
	reconcilerOpts := []managed.ReconcilerOption{
		managed.WithExternalConnector(&connector{kube: mgr.GetClient(), newGitlabClientFn: projects.NewServiceAccountAccessTokenClient}),
...
type connector struct {
	kube              client.Client
	newGitlabClientFn func(cfg common.Config) projects.ServiceAccountAccessTokenClient
}
...
func (e *external) observeSelf(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalObservation, error) {
	...
	cr.Status.AtProvider = projects.GenerateServiceAccountAccessTokenObservation(at)
	...
	if projects.ShouldRotateServiceAccountAccessToken(&cr.Spec.ForProvider, at) {
...
// observeOwner observes the token via the project-owner list endpoint.
func (e *external) observeOwner(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalObservation, error) {
	...
	at, res, err := projects.GetServiceAccountAccessToken(
		e.client,
...
func (e *external) createSelf(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalCreation, error) {
	at, _, err := e.client.RotateServiceAccountSelf(
		projects.GenerateRotateServiceAccountSelfOptions(&cr.Spec.ForProvider),
		gitlab.WithContext(ctx),
	)
	if err != nil {
...
// createOwner creates or rotates the token via the project-owner endpoints.
func (e *external) createOwner(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalCreation, error) {
	...
	if existingID, err := strconv.ParseInt(meta.GetExternalName(cr), 10, 64); err == nil && existingID > 0 {
		at, rotRes, rotErr := e.client.RotateProjectServiceAccountPersonalAccessToken(
			*cr.Spec.ForProvider.ProjectID,
			*cr.Spec.ForProvider.ServiceAccountID,
			existingID,
			projects.GenerateRotateServiceAccountAccessTokenOptions(&cr.Spec.ForProvider),
			gitlab.WithContext(ctx),
		)
		if rotErr == nil {
			meta.SetExternalName(cr, strconv.FormatInt(at.ID, 10))
	...
	at, _, err := e.client.CreateProjectServiceAccountPersonalAccessToken(
		*cr.Spec.ForProvider.ProjectID,
		*cr.Spec.ForProvider.ServiceAccountID,
		projects.GenerateCreateServiceAccountAccessTokenOptions(&cr.Spec.ForProvider),
		gitlab.WithContext(ctx),
	)
	if err != nil {
...
func (e *external) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	...
	_, err = e.client.RevokeProjectServiceAccountPersonalAccessToken(
		*cr.Spec.ForProvider.ProjectID,
		*cr.Spec.ForProvider.ServiceAccountID,
		int64(accessTokenID),
		gitlab.WithContext(ctx),
