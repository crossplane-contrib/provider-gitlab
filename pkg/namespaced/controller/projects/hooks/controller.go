...
func SetupHook(mgr ctrl.Manager, o controller.Options) error {
	...
	reconcilerOpts := []managed.ReconcilerOption{
		managed.WithExternalConnector(&connector{kube: mgr.GetClient(), newGitlabClientFn: projects.NewHookClient}),
...
type connector struct {
	kube              client.Client
	newGitlabClientFn func(cfg common.Config) projects.HookClient
}
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	hookOptions := projects.GenerateCreateHookOptions(&cr.Spec.ForProvider, token)
