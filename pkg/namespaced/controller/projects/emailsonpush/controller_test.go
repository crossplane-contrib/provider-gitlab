...
func TestUpdate(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"SetError": {
			...
			want: want{
				err: errors.Wrap(errBoom, errSetFailed),
			},
		},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			if diff := cmp.Diff(tc.want.err, err, test.EquateErrors()); diff != "" {
				t.Fatalf("Update error mismatch (-want +got):\n%s", diff)
			}
		})
	}
}
...
func TestCreate(t *testing.T) {
	e := &external{}
	res, err := e.Create(context.Background(), emailOnPush())

	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if diff := cmp.Diff(managed.ExternalCreation{}, res); diff != "" {
		t.Fatalf("Create mismatch: %s", diff)
	}
}

func TestDelete(t *testing.T) {
	e := &external{}
	res, err := e.Delete(context.Background(), emailOnPush())

	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if diff := cmp.Diff(managed.ExternalDelete{}, res); diff != "" {
		t.Fatalf("Delete mismatch: %s", diff)
	}
}
