...
func SetupBadge(mgr ctrl.Manager, o controller.Options) error {
	...
	reconcilerOpts := []managed.ReconcilerOption{
		managed.WithExternalConnector(&connector{kube: mgr.GetClient(), newGitlabClientFn: groups.NewBadgeClient}),
...
type connector struct {
	kube              client.Client
	newGitlabClientFn func(cfg common.Config) groups.BadgeClient
}
...
func (e *external) Observe(ctx context.Context, mg resource.Managed) (managed.ExternalObservation, error) {
	...
	cr.Status.AtProvider = groups.GenerateBadgeObservation(badge)
	cr.Status.SetConditions(v2.Available())

	return managed.ExternalObservation{
		ResourceExists:          true,
		ResourceUpToDate:        groups.IsBadgeUpToDate(&cr.Spec.ForProvider, badge),
...
func (e *external) Create(ctx context.Context, mg resource.Managed) (managed.ExternalCreation, error) {
	...
	badge, _, err := e.client.AddGroupBadge(
		*cr.Spec.ForProvider.GroupID,
		groups.GenerateAddGroupBadgeOptions(&cr.Spec.ForProvider),
...
func lateInitializeBadge(in *v1alpha1.BadgeParameters, badge *gitlab.GroupBadge) {
	if badge == nil {
		return
	}
