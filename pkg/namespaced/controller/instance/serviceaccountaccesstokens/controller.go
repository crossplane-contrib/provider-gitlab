...
const (
	errNotServiceAccountAccessToken = "managed resource is not a Gitlab instance service account access token custom resource"
...
const (
	...
	// ReasonOwnerManaged is set when the token is managed via instance-admin endpoints.
...
// instance ServiceAccountAccessTokens.
func SetupServiceAccountAccessToken(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(v1alpha1.ServiceAccountAccessTokenGroupKind)

	reconcilerOpts := []managed.ReconcilerOption{
		managed.WithExternalConnector(&connector{kube: mgr.GetClient(), newGitlabClientFn: instance.NewServiceAccountAccessTokenClient}),
...
type connector struct {
	kube              client.Client
	newGitlabClientFn func(cfg common.Config) instance.ServiceAccountAccessTokenClient
}
...
func (e *external) observeSelf(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalObservation, error) {
	...
	cr.Status.AtProvider = instance.GenerateServiceAccountAccessTokenObservation(at)
	...
	if instance.ShouldRotateServiceAccountAccessToken(&cr.Spec.ForProvider, at) {
...
// observeOwner observes the token via the admin get-by-id endpoint.
func (e *external) observeOwner(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalObservation, error) {
	...
	accessTokenID, err := strconv.ParseInt(externalName, 10, 64)
	if err != nil {
		return managed.ExternalObservation{}, errors.Wrap(err, errFailedParseID)
	}

	at, res, err := e.client.GetSinglePersonalAccessTokenByID(accessTokenID, gitlab.WithContext(ctx))
	if err != nil {
	...
	// Defensive: the external name must reference a token that belongs to the
	// configured service account. A mismatch means the token was replaced
	// out-of-band; treat it as non-existent so a fresh token is issued.
	if at == nil || (cr.Spec.ForProvider.ServiceAccountID != nil && at.UserID != *cr.Spec.ForProvider.ServiceAccountID) {
		return managed.ExternalObservation{ResourceExists: false}, nil
	}
...
func (e *external) createOwner(ctx context.Context, cr *v1alpha1.ServiceAccountAccessToken) (managed.ExternalCreation, error) {
	...
	if existingID, err := strconv.ParseInt(meta.GetExternalName(cr), 10, 64); err == nil && existingID > 0 {
		...
		// Only fall through to a fresh create when the token no longer exists
		// (revoked or deleted out-of-band). Any other rotation error is surfaced
		// so we never silently leave a still-valid token behind (which would
		// orphan it on the GitLab side).
		if !clients.IsResponseNotFound(rotRes) {
...
func (e *external) Delete(ctx context.Context, mg resource.Managed) (managed.ExternalDelete, error) {
	...
	_, err = e.client.RevokePersonalAccessTokenByID(accessTokenID, gitlab.WithContext(ctx))
