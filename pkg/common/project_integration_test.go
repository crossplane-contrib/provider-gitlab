	...
	cases := map[string]struct {
		...
		"AllFieldsSet": {
			...
			want: v1alpha1.CommonIntegrationObservation{
				...
				ConfidentialIssuesEvents:       ptr.To(testConfidentialIssuesEvents),
				ConfidentialNoteEvents:         ptr.To(testConfidentialNoteEvents),
				DeploymentEvents:               ptr.To(testDeploymentEvents),
				GroupConfidentialMentionEvents: ptr.To(testGroupConfidentialMentionEvents),
				GroupMentionEvents:             ptr.To(testGroupMentionEvents),
				IncidentEvents:                 ptr.To(testIncidentEvents),
				IssuesEvents:                   ptr.To(testIssuesEvents),
				JobEvents:                      ptr.To(testJobEvents),
				MergeRequestsEvents:            ptr.To(testMergeRequestsEvents),
				NoteEvents:                     ptr.To(testNoteEvents),
				PipelineEvents:                 ptr.To(testPipelineEvents),
				PushEvents:                     ptr.To(testPushEvents),
				TagPushEvents:                  ptr.To(testTagPushEvents),
				VulnerabilityEvents:            ptr.To(testVulnerabilityEvents),
				WikiPageEvents:                 ptr.To(testWikiPageEvents),
				CommentOnEventEnabled:          ptr.To(testCommentOnEventEnabled),
				Inherited:                      ptr.To(testInherited),
			},
		},
		"MinimalFields": {
			args: args{
